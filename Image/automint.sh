...
function getTextWithGramarFromWebsite() 
{
    echo $(wget --quiet -O - $2 | grep -o $1 | head -1)
}

function getSourceForgeDirectLinkWithLink()
{
    local SrcLink="${1/sourceforge.net/$2.sourceforge.net}"
    SrcLink="${SrcLink/projects/project}"
    SrcLink="${SrcLink/files\//}"
    echo "http://$SrcLink"
}
